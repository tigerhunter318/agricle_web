...
@section('content')
    <div class="mb-4">
        <h3>{{__('messages.recruitment.status.working')}}</h3>
    </div>
                ...
                <div class="col-lg-9 col-md-8 col-sm-12">
                    ...
                    <div class="mt-2 text-end">
                        <form id="form" method="POST">
                            ...
                            @if(time() > strtotime($recruitment['work_date_start']))
                                <button
                                    type="button"
                                    class="btn btn-success mx-1"
                                    data-bs-toggle="tooltip"
                                    data-bs-placement="bottom"
                                    title="{{ __('messages.recruitment.complete_work_tooltip') }}"
                                    data-container="body"
                                    data-animation="true"
                                    onclick="complete_work('{{ route('set_recruitment_status', ['id' => $recruitment->id, 'status' => 'completed']) }}')"
                                >
                                    <i class="fa fa-check"></i> {{__('messages.action.complete')}}
                                </button>
                                <a
                            ...
                            @endif
                            <a
                                href="{{ route('recruitment_detail_view', ['recruitment_id' => $recruitment['id']]) }}"
                                type="button"
                            ...
                            <button
                                ...
                                data-bs-toggle="tooltip"
                                data-bs-placement="bottom"
                                title="{{ __('messages.recruitment.cancel_work_tooltip') }}"
                                data-container="body"
                                data-animation="true"
                                onclick="cancel_work('{{ route('set_recruitment_status', ['id' => $recruitment->id, 'status' => 'canceled']) }}')"
                            >
                                <i class="fa fa-close"></i> {{__('messages.action.stop')}}
                            </button>
                            <a
...
@section('scripts')
    ...
    <script>
        function complete_work(url) {
            swal({
                title: "{{__('messages.alert.confirm')}}",
                text: "{{__('messages.alert.are_you_sure_to_complete_work')}}",
                type: 'warning',
        ...
        function cancel_work(url) {
            swal({
                title: "{{__('messages.alert.are_you_sure_to_cancel_work')}}",
