...
@section('content')
    ...
    <div class="mb-4">
        <h3>{{__('messages.title.recruitment_create')}}</h3>
    </div>

    <form enctype='multipart/form-data' method="POST" action="{{ route('recruitments.store') }}" id="form" autocomplete="off">
        @csrf
        <input type="hidden" name="status" value="draft" id="status" />
        <div class="row">
            <div class="col-md-3 col-sm-6 text-center">
                <img src="{{ old('image') ? old('image') : asset('assets/img/utils/empty.png') }}" id="previewImg" class="img-thumbnail img-fluid me-2" style="width: 200px; display: inline;" />
                <div class="m-2">
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="input-group input-group-outline mt-3">
                            <textarea name="description" class="form-control" type="text" rows="3" placeholder="{{__('messages.recruitment.description')}}">{{ old('description') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                <div class="input-group input-group-static }} mt-3">
                    ...
                    <select name="reward_type" id="reward_type" class="form-control">
                        <option value="{{__('messages.recruitment.reward.hourly')}}" {{ old('reward_type') == __('messages.recruitment.reward.hourly') ? "selected" : "" }}>{{__('messages.recruitment.reward.hourly')}}</option>
                        <option value="{{__('messages.recruitment.reward.daily')}}" {{ old('reward_type') == __('messages.recruitment.reward.daily') ? "selected" : "" }}>{{__('messages.recruitment.reward.daily')}}</option>
                    </select>
                </div>
                @error('reward_type')
                <small class="text-danger">{{ $message }}</small>
            ...
            <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                <div class="input-group input-group-static }} mt-3">
                    ...
                    <input type="number" class="form-control" name="reward_cost" value="{{ old('reward_cost') }}" step=any min="0">
                </div>
                @error('reward_cost')
                <small class="text-danger">{{ $message }}</small>
                @enderror
            ...
            <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                <input type="hidden" name="work_date_start" id="work_date_start">
                <input type="hidden" name="work_date_end" id="work_date_end">
                <div class="input-group input-group-static }} mt-3">
                    <label>{{__('messages.recruitment.work_date')}}</label>
                    <input type="text" class="form-control" name="work_date" id="work_date" value="{{ old('work_date') }}">
                </div>
            ...
            <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                <input type="hidden" name="work_time_start" id="work_time_start">
                <input type="hidden" name="work_time_end" id="work_time_end">
                <div class="input-group input-group-static }} mt-3">
                    <label>{{__('messages.recruitment.work_time')}}</label>
                    <input type="text" class="form-control" name="work_time" id="work_time" value="{{ old('work_time') }}">
                </div>
            ...
            <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                <div class="input-group input-group-static }} mt-3">
                    ...
                    <input type="text" class="form-control" name="break_time" id="break_time" value="{{ old('break_time') }}">
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                <div class="input-group input-group-static }} mt-3">
                    ...
                    <select name="lunch_mode" id="lunch_mode" class="form-control">
                        <option value="1" {{ old('lunch_mode') == '1' ? "selected" : "" }}>{{__('messages.recruitment.lunch_mode.yes')}}</option>
                        <option value="0" {{ old('lunch_mode') == '0' ? "selected" : "" }}>{{__('messages.recruitment.lunch_mode.no')}}</option>
                    </select>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                <div class="input-group input-group-static }} mt-3">
                    ...
                    <select name="pay_mode" id="pay_mode" class="form-control">
                        <option value="cash" {{ old('pay_mode') == 'cash' ? "selected" : "" }}>{{__('messages.recruitment.pay_mode.cash')}}</option>
                        <option value="card" {{ old('pay_mode') == 'card' ? "selected" : "" }}>{{__('messages.recruitment.pay_mode.card')}}</option>
                    </select>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                <div class="input-group input-group-static }} mt-3">
                    ...
                    <input type="number" class="form-control" name="worker_amount" value="{{ old('worker_amount') }}" min="1">
                </div>
                @error('worker_amount')
                <small class="text-danger">{{ $message }}</small>
                @enderror
            ...
            <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                <div class="input-group input-group-static }} mt-3">
                    ...
                    <select name="traffic_type" id="traffic_type" class="form-control">
                        <option value="include" {{ old('traffic_type') == 'include' ? "selected" : "" }}>{{__('messages.recruitment.traffic.include')}}</option>
                        <option value="beside" {{ old('traffic_type') == 'beside' ? "selected" : "" }}>{{__('messages.recruitment.traffic.beside')}}</option>
                    </select>
                </div>
            </div>

            <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12" id="traffic_cost_div" style="display: {{ old('traffic_type') && old('traffic_type') == 'beside' ? 'inline' : 'none' }}">
                <div class="input-group input-group-static }} mt-3">
                    <label>{{__('messages.recruitment.traffic.cost')}}</label>
                    <input type="number" class="form-control" name="traffic_cost" value="{{ old('traffic_cost') }}" step=any min="0">
                </div>
                @error('traffic_cost')
                <small class="text-danger">{{ $message }}</small>
                @enderror
            ...
            <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                <div class="input-group input-group-static }} mt-3">
                    ...
                    <input type="text" class="form-control" name="rain_mode" value="{{ old('rain_mode') }}">
                </div>
            </div>
            ...
            <div class="col-md-12">
                <div class="input-group input-group-outline mt-3">
                    <textarea name="notice" class="form-control" type="text" rows="3" placeholder="{{__('messages.recruitment.notice')}}">{{ old('notice') }}</textarea>
                </div>
            </div>

            <div class="col-md-4 col-lg-4 col-sm-12">
                <div class="form-check mt-3">
                    <input class="form-check-input" type="checkbox" id="toilet" {{ old('toilet')==='on' ? 'checked' : '' }} name="toilet">
            ...
            <div class="col-md-4 col-lg-4 col-sm-12">
                <div class="form-check mt-3">
                    <input class="form-check-input" type="checkbox" id="park" {{ old('park')==='on' ? 'checked' : '' }} name="park">
            ...
            <div class="col-md-4 col-lg-4 col-sm-12">
                <div class="form-check mt-3">
                    <input class="form-check-input" type="checkbox" id="insurance" {{ old('insurance')==='on' ? 'checked' : '' }} name="insurance">
                    <label class="form-check-label" for="insurance">{{__('messages.recruitment.insurance.title')}}</label>
                </div>
            </div>

            <div class="col-12">
                <button type="button" class="btn btn-info m-1" id="to_collecting_btn" onclick="create('collecting')"> <i class="fa fa-users"></i> {{__('messages.recruitment.create')}} </button>
                <button type="button" class="btn btn-success m-1" id="to_draft_btn" onclick="create('draft')"> <i class="fa fa-edit"></i> {{__('messages.action.draft')}} </button>
...
@section('scripts')
    ...
    <script>
        ...
        if("{{old('clothes')}}") clothes_select.setValue(("{{old('clothes')}}").split(','));
        ...
        $("#clothes_select").on("change", function () {
            if($(this).val() === 'custom') {
                $("#clothes_select").css('display', 'none');
                $("#clothes_input").css('display', 'inline');
                $("#clothes_input").val("{{__('messages.recruitment.clothes.title')}}");
            }
        });

        $("#clothes_input").on("keyup", function () {
            if($(this).val() === '') {
                $("#clothes_select").css('display', 'inline');
                $("#clothes_input").css('display', 'none');
                $("#clothes_select").val("{{__('messages.recruitment.clothes.casual')}}");
            }
        });
